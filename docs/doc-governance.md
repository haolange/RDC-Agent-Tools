...
- 如果文档涉及 `rd.event.get_actions`，还必须写清它是 root/pass 级浏览入口，deep lookup 应继续交给 `rd.event.get_action_tree`。
...
- 如果文档同时涉及 screenshot / readback 验证，还必须说明 replacement 成功后它们应绑定同一份 live replay state，而不是各自使用独立的 event/target 推断。
