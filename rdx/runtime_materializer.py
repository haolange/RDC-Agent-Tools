...
def load_runtime_source() -> RuntimeSource:
    ...
    for entry in files:
        if not isinstance(entry, dict):
            raise RuntimeError(f"runtime manifest entry is not an object: {manifest_path}")
        rel = str(entry.get("path") or "").strip()
        if not rel:
            raise RuntimeError(f"runtime manifest contains an empty path: {manifest_path}")
...
def materialize_runtime() -> MaterializedRuntime:
    ...
    if marker_path.is_file():
        ...
        if (
            ...
            and str(marker.get("manifest_sha256") or "").strip() == source.manifest_sha256
